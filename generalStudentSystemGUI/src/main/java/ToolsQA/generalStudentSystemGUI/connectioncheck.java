...
public class connectioncheck {
	/**
	 * It takes a table name and a connection as input, and returns a 2D array of
	 * strings containing the table's data
	 * 
	...
	public static String[][] downloadTableAtt(String tableName, Connection con) throws SQLException {
		// Execute SQL query
		Statement statement = con.createStatement();
		ResultSet rs = statement.executeQuery("SELECT * FROM " + tableName + " ORDER BY ABS(Student_Id);");
	...
	/**
	 * For each row in the array, if the first element in the row is not null, print
	 * the row
	 * 
	...
	/**
	 * It takes a 2D array of strings, a connection to a database, and a table name,
	 * and then it updates the table with the values in the array
	 * 
	 * @param infos     a 2D array of strings, where the first row is the column
	 *                  names and the rest are the values
	 ...
	 * @param tableName The name of the table you want to update
	 */
	public static void uploadArray1(String[][] infos, Connection con, String tableName) throws SQLException {
		// Create SQL statement1
		try {
			for (int i = 1; i < infos[i].length; i++) {
				StringBuilder sql = new StringBuilder();
				sql.append("UPDATE " + tableName + " SET ");
				for (int j = 1; j < infos.length - 1; j++) {
					if (infos[j][0] != null) {
						int studentid = Integer.parseInt(infos[j][0]);
						System.out.println(studentid);
						sql.append(String.valueOf(studentid) + " = '" + infos[j][i] + "'");
						if (j != infos[i].length) {
							sql.append(", ");
						}
						sql.append("WHERE (" + infos[0][0] + " = '" + (infos[j][0]) + "'); ");
	...
	/**
	 * This function creates a table with the name of the first parameter, and the
	 * columns of the table are the strings in the second parameter
	 * 
	 * @param tableName    The name of the table you want to create
	 * @param columnTitles An array of Strings that represent the column titles of
	 *                     the table.
	...
	/**
	 * This function takes in a table name, a column name, and a connection to a
	 * database, and adds a column to the table with the given name
	 * 
	...
	public static void main(String[] args) {
		...
		try {
			...
			String[][] infos = downloadTableAtt("Attendance_ics4u1", con);
